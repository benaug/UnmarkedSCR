...
#Estimation is shit for negative binomial in unmarked SCR, but may be useful with informative priors
#lam0 and theta are fixed at simulated values in model file just to show this works.
#Make changes in model file if you change simulation settings or apply to real data.
...
source("NimbleModel USCR Multisession NegBin.R")
source("NimbleFunctions USCR Multisession NegBin.R")
...
#Simulate some data
data <- simUSCR.multisession(N.session=N.session,lambda=lambda,lam0=lam0,sigma=sigma,
                          theta=theta,K=K,X=X,buff=buff,obstype=obstype)
...
inits <- list(lam0=lam0,sigma=sigma,theta=theta)#initial values for lam0, sigma to build data. in practice, don't use simulated values as inits.
nimbuild <- init.data.USCR.multisession(data=data,M=M,inits=inits,obstype="negbin")
...
Niminits <- list(N=N.init,z=nimbuild$z,s=nimbuild$s,ID=nimbuild$ID,capcounts=apply(nimbuild$y.true2D,c(1,2),sum,na.rm=TRUE),
                 y.true=nimbuild$y.true2D,lam0.fixed=0.5,sigma.fixed=0.5,theta.fixed=0.05,D=0.5)
...
parameters <- c('lam0.fixed','sigma.fixed','theta.fixed','N','n','D','lambda')
...
for(g in 1:N.session){
  ...
  if(cluster.ups>0){
    lam.nodes <- Rmodel$expandNodeNames(paste("lam[",g,",","1:",M[g],",1:",J[g],"]"))
    p.nodes <- Rmodel$expandNodeNames(paste("p[",g,",","1:",M[g],",1:",J[g],"]"))
    calcNodes <- c(calcNodes,lam.nodes,p.nodes,paste("N[",g,"]")) #need N and lam nodes for joint update
  }
  j.indicator <- 1:J[g]%in%data$this.j[g,1:nimbuild$n.samples[g]]
  conf$addSampler(target = y.nodes,
                  type = 'IDSampler',control = list(g=g,M=M[g],J=J[g],this.j=data$this.j[g,1:nimbuild$n.samples[g]],
                                                    j.indicator=j.indicator,calcNodes=calcNodes,z.nodes=NA,
                                                    n.samples=nimbuild$n.samples[g],K1D=K1D[g,1:J[g]],
...
plot(mcmc(mvSamples[2000:nrow(mvSamples),]))
