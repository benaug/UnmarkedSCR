...
source("NimbleModel USCR NegBin.R")
...
#Simulate some data
data=simUSCR(N=N,lam0=lam0,sigma=sigma,theta=theta,K=K,X=X,buff=buff,obstype=obstype)
...
inits=list(lam0=0.8,sigma=0.4,theta=0.1,psi=0.5)#initial values for lam0, sigma, and psi to build data
nimbuild=init.data.USCR(data=data,M=M,inits=inits,obstype="negbin")

#inits for nimble
Niminits <- list(z=nimbuild$z,s=nimbuild$s,ID=nimbuild$ID,capcounts=rowSums(nimbuild$y.true2D),
                 y.true=nimbuild$y.true2D,lam0=inits$lam0,sigma=inits$sigma,theta=inits$theta)
...
parameters<-c('psi','lam0','sigma','theta','N','n')
...
conf$addSampler(target = paste0("y.true[1:",M,",1:",J,"]"),
                type = 'IDSampler',control = list(M=M,J=J,this.j=data$this.j,
