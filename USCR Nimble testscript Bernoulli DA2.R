...
source("NimbleModel USCR Bernoulli DA2.R")
source("NimbleFunctions USCR Bernoulli DA2.R")
source("sSampler.R")
...
inits <- list(p0=0.2,sigma=1)#ballpark initial values for p0 and sigma to build data
...
parameters <- c('lambda.N','p0','sigma','N','n')
nt <- 2 #thinning rate
...
conf$addSampler(target = paste0("y.true[1:",M,",1:",J,"]"),
                type = 'IDSampler',control = list(M=M,J=J,K=K,this.j=data$this.j,this.k=data$this.k,
                                                  n.samples=nimbuild$n.samples,IDups=IDups),
                silent = TRUE)
